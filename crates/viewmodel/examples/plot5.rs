...
fn main() {
    let root = SVGBackend::new("plot5.svg", (1200, 675)).into_drawing_area();
    ...
    let mut data2 = Vec::<CoordForce>::new();
    ...
    for _ in 0..iters {
        ...
        if let Some(ForceVector { attractive, .. }) = m1.force_vector(&all_members, phase.clone()) {
            // let range = 60.0;
            // let scaled = force.x / scale;
            // let point = scaled.clamp(-range, range);
            // let left = " ".repeat((point) as usize);

            // println!("{}\t{}* {}, {}", m1.coords().0, left, force.x, point);
        ...
        if let Some(ForceVector { attractive, .. }) = m2.force_vector(&all_members, phase.clone()) {
            data2.push(CoordForce {
                coord: m2.position().0,
                force: attractive.x.clamp(-200.0, 200.0),
            });
        }
    ...
    // chart.draw_series(LineSeries::new(d2, &MAGENTA)).unwrap();

    // chart
    //     .draw_series(LineSeries::new(
    //         d3,
    //         ShapeStyle {
    //             color: RGBColor(42, 157, 143).to_rgba(),
    //             filled: true,
    //             stroke_width: 1,
    //         },
    //     ))
